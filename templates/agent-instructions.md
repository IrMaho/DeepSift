...
5. **DEC_v2 Compression Priority:** By default, allow search results to be compressed to save tokens. Only use the `--no-compress` flag as a fallback if you struggle to decode/reconstruct the compressed output after multiple attempts, or when you need to inspect the exact literal syntax of a code snippet for precise copying.
