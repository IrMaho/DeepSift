...
| `deepsift edit "patch.toon"` | Apply replacements. **CRITICAL:** Use TOON-Patch v2 to eliminate old code duplication. Dictionary `[~token:value]`. Target files `📄 file/path`. **Format 1 (Zero Duplication):** Use `L10-L15` (or `L10`) to target exact lines without outputting the old code: `L10-L15` \n `====` \n `new code...` \n `>>>>`. **Format 2 (Granular Inline):** Replace a small string on a line: `L5:<<<<` \n `old_word` \n `====` \n `new_word` \n `>>>>`. Line numbers are safe because DeepSift sorts them bottom-up automatically! |
