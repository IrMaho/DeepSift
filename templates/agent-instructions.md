...
| `deepsift edit "patch.json"` | Apply a batch of string replacements across multiple files instantly. You can provide a flat array `[{ "file": "path", "edits": [{ "search": "old", "replace": "new" }] }]`. **CRITICAL FOR SAVING OUTPUT TOKENS:** You can also use a dynamic dictionary to compress your output: `{ "dictionary": { "~f": "function", "~c": "console" }, "files": [{ "file": "path", "edits": [{ "search": "~f()", "replace": "~f(arg)" }] }] }`. DeepSift will expand `search` and `replace` using your dictionary before applying. |
