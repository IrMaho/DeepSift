...
| `deepsift edit "patch.toon"` | Apply replacements across multiple files. **CRITICAL FOR SAVING OUTPUT TOKENS:** Always use the ultra-minimal TOON-Patch syntax. First line must be dictionary `[~token:value,...]`. Then target files with `📄 file/path`. Then edit blocks enclosed by `<<<<`, `====`, `>>>>`. Example:\n`[~f:function,~c:console.log]`\n`📄 src/app.ts`\n`<<<<`\n`~f old() { ~c("old"); }`\n`====`\n`~f new() { ~c("new"); }`\n`>>>>` |
