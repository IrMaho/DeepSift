...
- **Mount copied realms:** `deepsift realm mount` (Scans `.deepsift/realms` and auto-attaches orphaned DBs copied from other projects)
- **Snapshot realm:** `deepsift realm snapshot <id>` (Gets a full list of all files/chunks stored in that database)
