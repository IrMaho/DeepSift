...
export function formatDNASummary(dna: ProjectDNA): string {
    ...
    const tokenCounts = Object.entries(dna.designSystem.tokens)
        .map(([cat, tokens]) => {
            ...
            return { cat, count: isArray ? tokens.length : 0, samples: isArray ? tokens as DiscoveredToken[] : [] };
        })
        .filter(t => t.count > 0);
    if (tokenCounts.length > 0) {
        ...
        lines.push(`- **Sources:** ${Array.isArray(dna.designSystem.tokenSources) ? dna.designSystem.tokenSources.join(', ') : 'Unknown'}`);
