...
| **`scan`** | `<target>` | اجرای آنالیزورهای خاص DNA (مانند tokens, i18n, duplicates, conventions, assets). <br>نکته: بخش‌های **duplicates** (یافتن کدهای تکراری بر اساس شباهت) و **conventions** (بررسی قوانین نام‌گذاری) کاملاً پیاده‌سازی و فعال شده‌اند. |
...
| **`deps`** | `"target_module"` | ردیابی و یافتن تمامی فایل‌هایی که ماژول هدف را ایمپورت یا به آن ارجاع داده‌اند (پشتیبانی از TS/JS و HTML/Nginx). |
